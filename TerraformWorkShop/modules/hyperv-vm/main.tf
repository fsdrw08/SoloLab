...
resource "hyperv_machine_instance" "vm_instance" {
  depends_on = [hyperv_vhd.boot_disk]
  ...
  dynamic "network_adaptors" {
    ...
    # for_each = var.network_adaptors == null ? [] : flatten([var.network_adaptors])
    content {
  ...
  dynamic "hard_disk_drives" {
    ...
    for_each = local.all_disk_drives
    content {
      controller_type                 = lookup(hard_disk_drives.value, "controller_type", null)
      controller_number               = lookup(hard_disk_drives.value, "controller_number", null)
      controller_location             = lookup(hard_disk_drives.value, "controller_location", null)
      path                            = lookup(hard_disk_drives.value, "path", null)
      disk_number                     = lookup(hard_disk_drives.value, "disk_number", null)
      resource_pool_name              = lookup(hard_disk_drives.value, "resource_pool_name", null)
      support_persistent_reservations = lookup(hard_disk_drives.value, "support_persistent_reservations", null)
      maximum_iops                    = lookup(hard_disk_drives.value, "maximum_iops", null)
      minimum_iops                    = lookup(hard_disk_drives.value, "minimum_iops", null)
      qos_policy_id                   = lookup(hard_disk_drives.value, "qos_policy_id", null)
      override_cache_attributes       = lookup(hard_disk_drives.value, "override_cache_attributes", null)
    }
  }
}
