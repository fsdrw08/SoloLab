services {
  ...
  tags = [
    ...
    "traefik.http.routers.metrics.rule=(Host(`traefik.day2.sololab`)||Host(`day2.traefik.service.consul`)) && PathPrefix(`/metrics`)",
    "traefik.http.routers.metrics.tls=true",
    "traefik.http.routers.metrics.service=prometheus@internal",
  ]
  meta = {
    prom_blackbox_scheme            = "https"
    prom_blackbox_address           = "day2.traefik.service.consul"
    prom_blackbox_health_check_path = "metrics"

    prom_target_scheme       = "https"
    prom_target_address      = "day2.traefik.service.consul"
