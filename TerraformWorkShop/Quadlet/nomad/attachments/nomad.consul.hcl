...
services {
  ...
  tags = [
    ...
    "traefik.http.routers.nomad-redirect.rule=Host(`nomad.day2.sololab`)||Host(`nomad.service.consul`)",
    "traefik.http.routers.nomad-redirect.middlewares=toHttps@file",
    "traefik.http.routers.nomad.entrypoints=webSecure",
    "traefik.http.routers.nomad.rule=Host(`nomad.day2.sololab`)||Host(`nomad.service.consul`)",
    "traefik.http.routers.nomad.tls=true",
    "traefik.http.services.nomad.loadBalancer.serversTransport=sololab-day2@file",
    "traefik.http.services.nomad.loadbalancer.server.scheme=https",
  ]
  meta = {
    prom_blackbox_scheme            = "https"
    prom_blackbox_address           = "nomad.day2.sololab"
    prom_blackbox_health_check_path = "v1/status/leader"

    prom_target_scheme                    = "https"
    prom_target_address                   = "nomad.day2.sololab"
