...
job "gitlab-db" {
  ...
  group "gitlab-db" {
    ...
    task "gitlab-db-main" {
      ...
      service {
        ...
        meta {
          ...
          dbConfig = "host=${NOMAD_TASK_NAME}-${NOMAD_ALLOC_ID} dbname=gitlabhq_production auth_user=pgbouncer"
          ...
          # https://prometheus-postgres-exporter.service.consul/probe?auth_module=postgres_exporter&target=postgresql-day3.service.consul%3A5432%2Fgitlab
          ...
          prom_target_metrics_path_param_target      = "pgbouncer.service.consul:6432/gitlabhq_production"
        }
      }
      ...
      driver = "podman"
      config {
        image = "zot.day1.sololab/fedora/postgresql-18:20260715"
        volumes = [
      ...
      template {
        ...
        host  all           {{with secret "kvv2_pgsql/data/gitlab"}}{{.Data.data.user_name}}{{end}}       all           scram-sha-256
        EOH
      ...
      template {
        ...
        -- create pgbouncer role and create function for auth
      ...
      template {
        ...
        POSTGRESQL_USER={{with secret "kvv2_pgsql/data/gitlab"}}{{.Data.data.user_name}}{{end}}
        POSTGRESQL_PASSWORD={{with secret "kvv2_pgsql/data/gitlab"}}{{.Data.data.user_password}}{{end}}
        POSTGRESQL_ADMIN_PASSWORD={{with secret "kvv2_pgsql/data/gitlab"}}{{.Data.data.admin_password}}{{end}}
        EOH
    ...
    volume "gitlab-db-main" {
      type            = "host"
      source          = "hvol-gitlab-db-main"
      read_only       = false
      attachment_mode = "file-system"
      access_mode     = "single-node-writer"
    }

    task "gitlab-db-registry" {
      # https://developer.hashicorp.com/nomad/docs/job-specification/service
      service {
        provider = "consul"
        name     = "gitlab-db-registry"
        ...
        meta {
          ...
          dbConfig = "host=${NOMAD_TASK_NAME}-${NOMAD_ALLOC_ID} dbname=gitlabhq_registry auth_user=pgbouncer"
          # meta data to render pgweb config with consul template
          dbUser        = "gitlab"
          pgBouncerHost = "pgbouncer.service.consul"
          # meta data for Prometheus consul_sd_config:
          ...
          prom_target_metrics_path                   = "probe"
          prom_target_metrics_path_param_auth_module = "postgres_exporter"
          prom_target_metrics_path_param_target      = "pgbouncer.service.consul:6432/gitlabhq_registry"
        }
      }
      ...
      # https://developer.hashicorp.com/nomad/docs/job-specification/env
      env {
        POSTGRESQL_DATABASE        = "gitlabhq_registry"
        POSTGRESQL_LOG_DESTINATION = "/dev/stderr"
      }
    ...
    volume "gitlab-db-registry" {
      type            = "host"
      source          = "hvol-gitlab-db-registry"
      read_only       = false
      attachment_mode = "file-system"
      access_mode     = "single-node-writer"
    }
