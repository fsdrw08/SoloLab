...
resource "vault_mount" "pki_ica_v1" {
  path                      = "pki/ica1_v1"
  type                      = "pki"
  description               = "PKI engine hosting intermediate CA v1 for sololab"
...
data "vault_pki_secret_backend_issuers" "pki_ica_v1" {
  backend = vault_mount.pki_ica_v1.path
}

resource "vault_pki_secret_backend_issuer" "pki_ica_v1" {
  count                          = data.vault_pki_secret_backend_issuers.pki_ica_v1.key_info == null ? 0 : 1
  backend                        = vault_mount.pki_ica_v1.path
  issuer_ref                     = element(keys(data.vault_pki_secret_backend_issuers.pki_ica_v1.key_info), 0)
