...
(`traefik.day2.sololab` in [TerraformWorkShop/etcd/skydns](../../TerraformWorkShop/etcd/skydns/terraform.tfvars))
```powershell
$repoDir=git rev-parse --show-toplevel
...
- Ensure Traefik container image had synced to image server
...
(`consul-client.day1.sololab` in [TerraformWorkShop/etcd/skydns](../../TerraformWorkShop/etcd/skydns/terraform.tfvars))
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="TerraformWorkShop/etcd/skydns/"
terraform -chdir="$(Join-Path -Path $repoDir -ChildPath $childPath)" apply -auto-approve
```
#### Workload Pre-request:
- Ensure consul container image had synced to image server
(in [LocalWorkShop/Sync-OCIImage/Day1.jsonc](../../LocalWorkShop/Sync-OCIImage/Day1.jsonc))
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="LocalWorkShop/Sync-OCIImage"
Set-Location -Path (Join-Path -Path $repoDir -ChildPath $childPath)
...
vault kv put -address $vaultAddress -mount=kvv2_nomad token-management token=$env:NOMAD_TOKEN
```
[TerraformWorkShop/Nomad/ACL](../../TerraformWorkShop/Nomad/ACL/) in terminal, run
```powershell
$vaultAddress = "https://vault.day1.sololab"
vault login -no-print -method=ldap -address $vaultAddress username=$($credential.UserName) password=$($credential.GetNetworkCredential().Password)
