...
Check pre-requirements: [TerraformWorkShop/Quadlet/nomad](../../TerraformWorkShop/Quadlet/nomad/)
```powershell
$repoDir=git rev-parse --show-toplevel
...
Check pre-requirements: [TerraformWorkShop/Nomad/ACL](../../TerraformWorkShop/Nomad/ACL/)
```powershell
$repoDir=git rev-parse --show-toplevel
...
Check pre-requirements: [TerraformWorkShop/Vault/Auth/JWT](../../TerraformWorkShop/Vault/Auth/JWT)
```powershell
$credential = Get-Credential -Message "credential to login vault" -UserName "000"
$env:VAULT_ADDR = "https://vault.day1.sololab"
vault login -no-print -method=ldap username=$($credential.UserName) password=$($credential.GetNetworkCredential().Password)
