...
terraform -chdir="$(Join-Path -Path $repoDir -ChildPath $childPath)" apply -auto-approve
```
- Ensure postgresql related credential is ready for Gitea is ready  
[TerraformWorkShop/Vault/Secrets/PostgreSQL](../../TerraformWorkShop/Vault/Secrets/PostgreSQL/terraform.tfvars)
```powershell
$repoDir=git rev-parse --show-toplevel
...
```
#### Workload Pre-request:
- Ensure OTF container image `docker.io/leg100/otfd:0.5.24` had synced to image server  
[LocalWorkShop/Sync-OCIImage/Day4.jsonc](../../LocalWorkShop/Sync-OCIImage/Day4.jsonc)
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="LocalWorkShop/Sync-OCIImage"
...
terraform -chdir="$(Join-Path -Path $repoDir -ChildPath $childPath)" apply -auto-approve
```
- Ensure postgresql related credential is ready for OTF  
...
terraform -chdir="$(Join-Path -Path $repoDir -ChildPath $childPath)" apply -auto-approve
```
- Ensure OTF site secret in Vault is ready  
...
#### Deploy OTF nomad job
[TerraformWorkShop/Nomad/Jobs/otf/](../../TerraformWorkShop/Nomad/Jobs/otf/)
```powershell
$credential = Get-Credential -Message "credential to login vault" -UserName "000"
$env:VAULT_ADDR = "https://vault.day1.sololab"
vault login -no-print -method=ldap username=$($credential.UserName) password=$($credential.GetNetworkCredential().Password)
