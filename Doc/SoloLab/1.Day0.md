### Provision VyOS VM image by packer
...
.\Invoke-PackerBuild.ps1 'stream-cloudinit'
```
...
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="LocalWorkShop/.ssh"
$From=Join-Path -Path $repoDir -ChildPath $childPath
$To=$env:USERPROFILE
...
### Create and trust private root CA
...
### Create VyOS VM
[TerraformWorkShop/Hyper-V/VM-Day0-VyOS](../../TerraformWorkShop/Hyper-V/VM-Day0-VyOS/)  
Then config shellcrash if need [TerraformWorkShop\Hyper-V\VM-Day0-VyOS\shellclash\README.md](../../TerraformWorkShop/Hyper-V/VM-Day0-VyOS/shellclash/README.md)
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="TerraformWorkShop/Hyper-V/VM-Day0-VyOS"
Set-Location -Path (Join-Path -Path $repoDir -ChildPath $childPath)
sudo pwsh.exe -c "terraform init -upgrade";
terraform apply -auto-approve
```

### Deploy zot image registry in VyOS VM
...
$tfstates=@(
    ...
    "vyos_config_block_tree.reverse_proxy[`"l4_frontend2`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_backend`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_frontend`"]",
    "module.vyos_container.null_resource.load_image[`"zot`"]",
    "module.vyos_container.vyos_config_block_tree.container_network[0]",
    "module.vyos_container.vyos_config_block_tree.container_workload[`"zot`"]"
)
...
### Sync container image to zot
[LocalWorkShop/Sync-OCIImage/Day0.jsonc](../../LocalWorkShop/Sync-OCIImage/Day0.jsonc)
```powershell
$repoDir=git rev-parse --show-toplevel
...
.\Sync-OCIImage.ps1 -SyncProfile "Day0.jsonc"  -PrivateRegistry "zot.day0.sololab.dev"
# .\Sync-OCIImage.ps1 -SyncProfile "Day0.jsonc" -LocalStore "D:/Users/Public/Downloads/containers" -PrivateRegistry "zot.day0.sololab.dev"
```
...
Set-Location -Path (Join-Path -Path $repoDir -ChildPath $childPath)
$proxy="127.0.0.1:7890"; $env:HTTP_PROXY=$proxy; $env:HTTPS_PROXY=$proxy; $env:NO_PROXY="sololab"
.\Sync-TrivyDB.ps1 -SyncProfile "Day0-Trivy.jsonc" # -Upload $false
# .\Sync-TrivyDB.ps1 -SyncProfile "Day0-Trivy.jsonc" -LocalStore "D:/Users/Public/Downloads/containers/trivy" -Upload $false
```
...
### Deploy PowerDNS server in VyOS
...
$tfstates=@(
    ...
    "vyos_config_block_tree.reverse_proxy[`"l7_backend`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_frontend`"]",
    "module.vyos_container.null_resource.load_image[`"powerdns`"]",
    "module.vyos_container.vyos_config_block_tree.container_network[0]",
    "module.vyos_container.vyos_config_block_tree.container_workload[`"powerdns`"]"
)
foreach ($tfstate in $tfstates) {
    terraform state rm $tfstate
}
...
### 14. Init Lynx
1. Go to https://lynx.day0.sololab, input:
```yaml
Application Name: Lynx
Application URL: https://lynx.day0.sololab
...
terraform state list
$tfstates=@(
    "vyos_config_block_tree.reverse_proxy[`"l4_backend_api`"]",
    "vyos_config_block_tree.reverse_proxy[`"l4_backend_console`"]",
    "vyos_config_block_tree.reverse_proxy[`"l4_frontend_api`"]",
    "vyos_config_block_tree.reverse_proxy[`"l4_frontend_console`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_backend_api`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_backend_console`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_frontend_api`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_frontend_console`"]",
    "module.vyos_container.null_resource.load_image[`"minio`"]",
    "module.vyos_container.vyos_config_block_tree.container_network[0]",
    "module.vyos_container.vyos_config_block_tree.container_workload[`"minio`"]"
)
foreach ($tfstate in $tfstates) {
    terraform state rm $tfstate
}
...
### Init MinIO
[TerraformWorkShop/MinIO/Day0](../../TerraformWorkShop/MinIO/Day0)
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="TerraformWorkShop/MinIO/Day0"
Set-Location -Path (Join-Path -Path $repoDir -ChildPath $childPath)
sudo pwsh.exe -c "terraform init -upgrade";
terraform apply -auto-approve
```

### Seed DNS record to PowerDNS
...
$tfstates=@(
    "vyos_config_block_tree.reverse_proxy[`"l4_backend`"]",
    "vyos_config_block_tree.reverse_proxy[`"l4_frontend`"]",
    "vyos_config_block_tree.snat",
    "module.vyos_container.null_resource.load_image[`"cockpit`"]",
    "module.vyos_container.vyos_config_block_tree.container_network[0]",
    "module.vyos_container.vyos_config_block_tree.container_workload[`"cockpit`"]"
)
foreach ($tfstate in $tfstates) {
    terraform state rm $tfstate
}
...
### Deploy node exporter in VyOS
[TerraformWorkShop/VyOS/Service-Monitoring](../../TerraformWorkShop/VyOS/Service-Monitoring)
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="TerraformWorkShop/VyOS/Service-Monitoring"
Set-Location -Path (Join-Path -Path $repoDir -ChildPath $childPath)
sudo pwsh.exe -c "terraform init -upgrade";
terraform apply -auto-approve
```
...
$tfstates=@(
    ...
    "vyos_config_block_tree.reverse_proxy[`"l7_backend`"]",
    "vyos_config_block_tree.reverse_proxy[`"l7_frontend`"]",
    "vyos_config_block_tree.service_monitoring[`"node_exporter`"]"
)
foreach ($tfstate in $tfstates) {
    terraform state rm $tfstate
}
...
### Deploy shellcrash in vyos
[TerraformWorkShop/VyOS/Container-Shellcrash](../../TerraformWorkShop/VyOS/Container-Shellcrash)
```powershell
$repoDir=git rev-parse --show-toplevel
$childPath="TerraformWorkShop/VyOS/Container-Shellcrash"
Set-Location -Path (Join-Path -Path $repoDir -ChildPath $childPath)
sudo pwsh.exe -c "terraform init -upgrade";
terraform apply -auto-approve
```
then get in to and config the shellcrash container, 
ref: https://github.com/juewuy/ShellCrash/tree/dev/docker#container-management%E5%AE%B9%E5%99%A8%E7%AE%A1%E7%90%86
```powershell
ssh vyos
connect container shellcrash
sh -l
# 6 管理配置文件 -> 2 在线获取配置文件(基于订阅提供者)
# 2 功能设置 -> 6 自定义端口及秘钥 -> 8 自定义本机host地址
# 9 更新与支持 -> 4 安装本地Dashboard面板 -> 1 安装zashboard面板(约2.2mb)
# 1 启动/重启服务
# visit https://shellcrash.day0.sololab/ui/
# host: shellcrash.day0.sololab, port: 443
# add below content into /etc/crontabs/root:
# do daily/weekly/monthly maintenance
# min   hour    day     month   weekday command
* * * * * test -z "$(pidof CrashCore)" && echo -e "0" | sh -l && /bin/ash /etc/ShellCrash/starts/start_legacy_wd.sh shellcrash #ShellCrash保守模式守护进程
```